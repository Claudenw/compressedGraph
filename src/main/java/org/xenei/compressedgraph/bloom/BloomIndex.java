...
public class BloomIndex {
	...
	private final static byte[][] MASK = {
			...
			{ 0x3, 0x7, 0xB, 0xF, }, // 0x3
			{ 0x4, 0x5, 0x6, 0x7, 0xC, 0xD, 0xE, 0xF, }, // 0x4
			{ 0x5, 0x7, 0xD, 0xF, }, // 0x5
			{ 0x6, 0x7, 0xE, 0xF, }, // 0x6
			{ 0x7, 0xF, }, // 0x7
	...
	public static ExtendedIterator<ByteBuffer> getIndexIterator(
			ByteBuffer pattern) {
		byte[] buff = new byte[pattern.capacity()];
		Arrays.fill(buff, (byte) 0xFF);
		return WrappedIterator.create(new BufferIterator(pattern, ByteBuffer
				.wrap(buff)));
	}
	...
	public static ExtendedIterator<ByteBuffer> getIndexIterator(byte pattern) {
		...
		return WrappedIterator.create(new BufferIterator(ByteBuffer.wrap(patt),
				ByteBuffer.wrap(max)));
	}

	public static ExtendedIterator<ByteBuffer> getIndexIterator(
			ByteBuffer pattern, ByteBuffer limit) {
				...
				for (int i = 0; i < pattern.capacity(); i++) {
					if (currentPos[i] + 1 < maxPos[i]) {
						...
						for (int j = 0; j < pattern.capacity(); j++) {
							next.put(
									j,
									getExpansionByte(pattern.get(j),
											currentPos[j]));
						}
						used = false;
